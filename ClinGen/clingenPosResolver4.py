...
def parseFromClinVar(idList):
    ...
    for e in entries:
        ...
        if len(idPosition) == 1:
            ...
            print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAH")
...
for r in rows[1:]:
    ...
    expressions = r[3].split(", ") #Splits the string in the cell containing expressions into multiple strings, each containing a single expression.
    results = findGRCh37Expression(expressions) #Finds the GRCh37 Expression, if it exists. If not, returns empty strings.
    key = results[0]
    expression = results[1]
    
    clinvarID = r[1] #Assigns ClinVar ID from proper cell.

    #Iterates through both the list of expressions and GRCh37 keys to determine which expression refers to the GRCh37 Assembly
    ...
    elif (key != "") & (expression != ""): #If the GRCh37 expression was found, extract information from it.
...
allValuesDict = {} #{ClinVar ID : [chrom, pos, ref, alt, parseMethod]} Will be used to store combined results of Clinvar API calls.
for i in allClinVarIds: #Iterates through all Clinvar IDs
    ...
    if (len(clinIDList) == 199): #Makes sure staging list does not get larger than 200
        valuesDict = parseFromClinVar(clinIDList) #Performs API call, returns segment of 'allValuesDict'
...
for r in rows[1:]: #Interates through rows.
    if len(r) < len(rows[0]): #Makes sure to use rows that have not yet had VCF info added (Rows containing Clinvar IDs).
        clinvarID = r[1] #Assigns ClinVar ID from proper cell. 
        values = ["DictMatchError", "DictMatchError", "DictMatchError", "DictMatchError", "DictMatchError"]
        ...
        if allValuesDict[id][4] == "PARSING REQUIRED": #Determines if API call failed to get the data for this specific entry.
            ...
            if (key == "") | (expression == ""):
                ...
                values = parseFromExpression(key, expression) #Performs parse using Ensembl Variant Recoder API 
        else:
            values = allValuesDict[id] #Pulls VCF info from combined dictionary if ClinVAr API call was successful.
...
manualRows = []
with open("clinGenPathogenicity_ManualFix.csv", encoding='utf-8') as manual: #"encoding='utf-8'" flag must be present. For whatever reason, file cannot be read without it.
...
if os.path.exists("clinGenPathogenicityFixedCoordinates.csv"):
        os.remove("clinGenPathogenicityFixedCoordinates.csv")
